...
describe('database row-level security contract', () => {
  ...
  it('updates preferences through an authenticated versioned field patch', () => {
    expect(schema).toMatch(/patch_user_preferences\([\s\S]+preferences_field_versions/i);
    expect(schema).toMatch(/where profiles\.id = auth\.uid\(\)\s+for update/i);
    expect(schema).toMatch(/PREFERENCE_CONFLICT:nutritionTargetsMode/i);
    expect(schema).toMatch(/revoke all on function public\.patch_user_preferences\(jsonb, jsonb\) from public/i);
    expect(schema).toMatch(/grant execute on function public\.patch_user_preferences\(jsonb, jsonb\) to authenticated/i);
  });
});
